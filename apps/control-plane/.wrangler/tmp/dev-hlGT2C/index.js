...
// .wrangler/tmp/bundle-j9xRvK/middleware-loader.entry.ts
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();

// .wrangler/tmp/bundle-j9xRvK/middleware-insertion-facade.js
init_modules_watch_stub();
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_process();
init_performance2();
...
async function addColumnIfMissing(db, table, ddl, columnName) {
...
__name(ensureSchemaOnce, "ensureSchemaOnce");
...
function shopFromSessionTokenPayload(payload) {
  const candidates = [payload.dest, payload.iss].filter(Boolean);
  for (const candidate of candidates) {
    try {
      ...
      if (host === "admin.shopify.com") {
        ...
        const slug = storeIdx >= 0 ? parts[storeIdx + 1] : void 0;
...
__name(shopFromSessionTokenPayload, "shopFromSessionTokenPayload");
...
app.get("/auth/callback", async (c) => {
  ...
  let tokenRes;
  try {
    tokenRes = await withStep(
      c,
      "shopify.exchangeAccessToken",
      () => exchangeAccessToken({
        shop,
        apiKey: c.env.SHOPIFY_API_KEY,
...
app.get("/api/applepay/domains", async (c) => {
  ...
  const rows = await withStep(
    c,
    "db.listMerchantDomainsByShop",
...
var handler = {
  ...
  fetch: /* @__PURE__ */ __name((request, env2, ctx) => {
...
app.onError((err, c) => {
  ...
  const wantsJson = accept.includes("application/json") || accept.includes("application/*+json") || xRequestedWith.toLowerCase() === "xmlhttprequest";
